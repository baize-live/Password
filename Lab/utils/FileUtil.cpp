...
bool File::isExist() {
    return 0 == _access(m_sFilePath.c_str(), 0);
}
...
void File::read(std::string &sContent) {
    std::fstream file;
    file.open(m_sFilePath, std::ios::in); //以只读方式打开文件
    char buf2[2048]; //存储读取的内容
    while (!file.eof()) {
        memset(buf2, 0, 2048);
        file.read(buf2, 2048);
        sContent.append(buf2);
    }
    file.close(); //关闭文件
}

void File::write(std::string &sContent, std::ios_base::openmode mode = std::ios::app) {
    std::fstream file;
    file.open(m_sFilePath, mode);
    file.write(sContent.c_str(), (int) sContent.size());
    file.close(); //关闭文件
}
